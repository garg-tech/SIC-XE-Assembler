...
string objectCode, writeData, currentRecord, modificationRecord = "M^", endRecord; 
...
// Function to read the intermediate file created after pass1
...
// Function to return the object code for the given instruction
string createObjectCodeFormat34()
{
    ...
    else if (getFlagFormat(operand) == '@')
    {
        ...
        { // Error handling for Indirect with index based addressing
            if (halfBytes == 3)
            {
                writeData = "Line " + to_string(lineNumber);
...
void writeTextRecord(bool lastRecord = false)
{
    ...
    else
    {
        ...
        if (opcode == "START" || opcode == "END" || opcode == "BASE" || opcode == "NOBASE" /******/)
        {
            /*DO nothing*/
        }
        else
...
void writeEndRecord(bool write = true)
{
    ...
    { // Make operand on end firstExecutableAddress
        ...
        firstExecutableAddress = stringHexToInt(SYMTAB["START"].address);
...
void pass2()
{
    ...
    writeToFile(ListingFile, "Line\tAddress\tLabel\tOPCODE\tOPERAND\tObjectCode");
    ...
    writeData = "H^" + expandString(label, 6, ' ', true) + '^' + intToStringHex(address, 6);
    writeToFile(objectFile, writeData);
